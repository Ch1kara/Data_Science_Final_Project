...
class Pokemon(pygame.sprite.Sprite):
    ...
    def hp_bar(self, x, y):
        ...
        text_rect = text.get_rect()
        text_rect.x, text_rect.y = x, y+30
        bar_rect = bar.get_rect()
        bar_rect.x, bar_rect.y = x, y
        current_bar_rect = current_bar.get_rect()
        current_bar_rect.x, current_bar_rect.y = x, y
...
while status != 'quit':
            ...
            if status == "title":
                ...
                for i in range(len(battle_poke.Moves)):
                    ...
                    if trainer.current_HP == 0:
                        ...
                        status = 'trainer turn'
    if status == "title":
        # only draws the stuff in the title_drawn if statement once to prevent flickering
        if not title_drawn:
            screen.fill(white)
    ...
    if status == 'selection':
        ...
        counter = 0
        # only draws the below things once so that there is no flickering
        if not select_drawn:
            screen.fill(white)
            ...
            pygame.draw.rect(screen, white, (10, 525, 980, 210))
            pygame.draw.rect(screen, black, (10, 525, 980, 210), 3)
            # black outline of each pokemon selection button
            for i in range(len(pokedex.party)):
    ...
    if status == 'player turn':
        ...
        # only draws the below things once to prevent flickering
        if not move_drawn:
            screen.fill(white)
            Background('images/battle_background.png', [0, 0])
            battle_poke.paint()
            trainer.paint()
            message('')
            # black outlines of move buttons
            for i in range(len(battle_poke.Moves)):
                pygame.draw.rect(screen, black, (posx[i] - 3, posy[i] - 3, 456, 106), 3)
            move_drawn = True
    ...
    if status == 'trainer faint':
        ...
        if len(pokedex.party) < 6:  # Checking if the player already has six pokemon.
            ...
            print(pokedex.party)
        trainer = None  # Resetting to make sure the old pokemon isn't attached to the ai
